...
function printHelp () {
  echo "Usage: "
  echo "  network.sh up|down|restart|generate|upgrade [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-i <imagetag>]"
  ...
  echo "        network.sh up -c mychannel -s couchdb -i 1.1.0-alpha"
...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' orderer0.orderer.prototype.rks.plus |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer0.orderer.prototype.rks.plus
  docker cp -a orderer0.orderer.prototype.rks.plus:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer0.orderer.prototype.rks.plus
  docker-compose $COMPOSE_FILES up -d --no-deps orderer0.orderer.prototype.rks.plus

  for PEER in peer0.org1.prototype.rks.plus peer1.org1.prototype.rks.plus peer0.org2.prototype.rks.plus peer1.org2.prototype.rks.plus; do
...
#  CURRENT_DIR=$PWD
#  cd crypto-config/peerOrganizations/org1.prototype.rks.plus/ca/
#  PRIV_KEY=$(ls *_sk)
#  cd "$CURRENT_DIR"
#  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
#  cd crypto-config/peerOrganizations/org2.prototype.rks.plus/ca/
#  PRIV_KEY=$(ls *_sk)
#  cd "$CURRENT_DIR"
...
# (``peer0.org1.prototype.rks.plus`` & ``peer0.org2.prototype.rks.plus``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile BasisChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID ${CHANNEL_NAME}
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  for ORG in Executor Producer Distributor Service Consumer Supervisor; do
      ...
      echo
      echo "#################################################################"
      echo "#######    Generating anchor peer update for $ORG - $ORGMSP "
      echo "#################################################################"
      set -x
      configtxgen -profile BasisChannel -outputAnchorPeersUpdate \
